...
export async function construirDte05(req: any, versionDte: number, emisorDb: any) {
  ...
  const receptorSanitized = {
    ...
    nombre: receptor?.nombre || "Receptor Nota Credito",
  ...
  if (needsGeneration) {
    numControl = await obtenerSiguienteCorrelativo('05', emisorDb.id, codEstable, codPunto, 1);
  }
